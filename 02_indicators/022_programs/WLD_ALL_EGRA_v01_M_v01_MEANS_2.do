							...
							foreach lv of local lev {
								local label: label (`trait') `lv'
									...
									*Setting survey structure
									...
									if inlist("`c'", "LBR") & inlist("`y'", "2010"){
										mean `indicator'`sub'`trait'`lv' 
										//this data is mi set, so svyset does not work
									}
									if inlist("`c'", "LBR") & inlist("`y'", "2013"){
										svyset
										svy: mean `indicator'`sub'`trait'`lv' 
									}
									else  if !inlist("`c'","XXX", "LBR"){
										svyset 
										svy: mean `indicator'`sub'`trait'`lv' 
										return list
										matrix list e(V)
