*This file is for Pakistan (Punjab) only, so it is only representative for Punjab
...
local region      = "PAK"   /* LAC, SSA, WLD or CNT such as KHM RWA */
  ...
  *if (_rc == 601) | (`overwrite_files') {
    ...
    *---------------------------------------------------------------------------
    ...
    /* NOTE: Some assessments will loop over `prefix'`cnt' (such as PIRLS, TIMSS),
       ...
       // Temporary copies of the 4 rawdatasets needed for each country (new section) * various datasets for each region in PAK 
	...
	*<_idgrade_> 
	gen idgrade = cb5b
	replace idgrade = idgrade + 5 if cb5a== 2 & cb5b <5
	replace idgrade = idgrade + 8 if cb5a== 3 
	...
	replace idgrade= . if idgrade==18 & schage== 4
	replace idgrade= . if idgrade==14 & schage== 5
	//it is weird here that so many students are in higher ed, but so little are in grade 11 & 12
	replace idgrade = . if  cb5b>90	| cb5a==9	
    *</_idgrade_>
