...
local region      = "MDG"   /* LAC, SSA, WLD or CNT such as KHM RWA */
  ...
  *if (_rc == 601) | (`overwrite_files') {
    ...
    *---------------------------------------------------------------------------
    ...
    /* NOTE: Some assessments will loop over `prefix'`cnt' (such as PIRLS, TIMSS),
       ...
       // Temporary copies of the 4 rawdatasets needed for each country (new section) * 1 dataset 
    ...
    *<_score_assessment_reading> 
	...
	egen read_comp_score2 =rowtotal(score_fl222a score_fl222b score_fl222c score_fl222d score_fl222e)
	replace read_comp_score2 = . if score_fl222a==. & score_fl222b==. & score_fl222c==. & score_fl222d==. & score_fl222e==.
	egen read_comp_score3 =rowtotal(score_fl122a score_fl122b score_fl122c score_fl122d score_fl122e)
	replace read_comp_score3 = . if score_fl122a==. & score_fl122b==. & score_fl122c==. & score_fl122d==. & score_fl122e==.
	egen read_comp_score_pct= rowmax(read_comp_score read_comp_score2 read_comp_score3)
	replace read_comp_score_pct= read_comp_score_pct /5
	...
	*<_idgrade_> 
	gen idgrade = cb5b
	replace idgrade = idgrade + 5 if cb5a== 2
	replace idgrade = idgrade + 9 if cb5a== 3
	replace idgrade = idgrade + 12 if cb5a== 4
	replace idgrade = . if idgrade ==8 & schage<=7
	replace idgrade = . if idgrade==7 & schage <= 5
	label define grade 13 "tertiary" 14 "tertiary" 15 "tertiary" 16 "tertiary" 17 "tertiary"
	label val idgrade grade
	label var idgrade "Grade" 
	replace idgrade = . if  cb5b>90
	//there is a category "higher", which I am currently labeling as tertiary education, however 2 out of 3 children are still quite young.
    *</_idgrade_>
