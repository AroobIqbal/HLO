...
local region      = "ZWE"   /* LAC, SSA, WLD or CNT such as KHM RWA */
 ...
 *</_score_assessment_reading>
	foreach i of var fl22* fl21b*{
	gen score_`i' = `i'
	replace score_`i' = 0 if `i'>=2
	replace score_`i' = . if `i'==.
	}
	egen read_comp_score =rowtotal(score_fl22a score_fl22b score_fl22c score_fl22f score_fl22e)
	replace read_comp_score = . if score_fl22a==. & score_fl22b==. & score_fl22c==. & score_fl22f==. & score_fl22e==.
	egen read_comp_score2 =rowtotal(score_fl21ba score_fl21bb score_fl21bc score_fl21bf score_fl21be)
	replace read_comp_score2 = . if score_fl21ba==. & score_fl21bb==. & score_fl21bc==. & score_fl21bf==. & score_fl21be==.
	egen read_comp_score_pct= rowmax(read_comp_score read_comp_score2)
	...
	*<_idgrade_> 
	gen idgrade = cb5b
	replace idgrade = idgrade + 7 if cb5a==3
